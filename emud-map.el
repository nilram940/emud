...
(defun emud-map-new-room ()
  (let ((map emud-curr-map)
    ...
    (cond
     ...
     ;; Check that we actually have a movement command and it is not
     ;; listed in the command blacklist this is likely rendered redundant
     ;; by the "or" checks below.
     ((and
       last-cmd
       (not (memq last-cmd emud-map-bad-commands))
      					; we have a good command
      
       ;; Check to ensure that we have actually moved to a
      ...
      ;; Attempt to find the destination room.
      (setq dest-room ;dest-room setq
	    (cond  
	     ;; Check the exit list of the source room for one that
	     ...
	     ;; Check the entrances of the source room for entrances
	     ...
	     ;; Check for coordinate labeled rooms and attempt to find
       ...
       ;; Verify the that the short descriptions match.
	 ...
	 (setq dest-room nil))
       ...
       ;; Verify that the exits match. Obvious exits can change
	 ...
	 (unless (and (car (emud-room-siblings source-room))
		      (not (eq (car (emud-room-siblings source-room)) 'uniq)))
...
(defun emud-map-follow-exit (cmd exits map short)
  ...
  (let ((exit (cdr (assq cmd exits)))
	...
	(car (cl-intersection exit sibling)))
       (t
	exit)))))
...
(defun emud-map-check-sibling-entrances (map source-room last-cmd dest-room)
  ...
  (let ((dest-siblings (emud-room-siblings dest-room))
    ...
    (unless (or (and (car dest-siblings)
		     (not (eq (car dest-siblings) 'uniq)))
		(and (car source-siblings)
		     (not (eq (car source-siblings) 'uniq)))
		(not (cddr source-siblings)))
	...
	(setq sibling-entrances
      ...
      (when (setq merge-list (cl-intersection source-siblings sibling-entrances))
	...
	;; Go through the merge list and attempt to merge each room with main
	(dolist (merge-number merge-list)
...
(defun emud-map-check-entrances (map room last-cmd short)
  ...
  (let ((siblings (gethash short (emud-map-sibling-hash map)))
    ...
    (and siblings rev-cmd (not (eq (car source-siblings) 'maze))
	 ...
	 (setq dest-numbers (cl-intersection sibling siblings))
...
(defun emud-map-merge-map-rooms (map main-room target-room)
  (let ((target-number (emud-room-number target-room))
	...
	(if (eq main-room target-room)
	  ...
	  ;; If an alias points to the target room, move it to the
	  ;; main room
	  ...
	  (setq merge-list 
		(nconc (emud-map-merge-rooms map main-room target-room) merge-list))
	  ...
	  (unless (memq target-number (emud-room-siblings main-room))
	    (setf (emud-map-hole map) 
		  (cons target-number (emud-map-hole map)))))))))
...
lead to room1."
  (let ((new-exit (emud-room-number room1)) 
    ...
    (setq entrance-alist (cl-union (emud-room-entrances room2) 
...
(defun emud-map-fix-entrances (map room1 room2)
  (let ((new-entrance (emud-room-number room1))
    ...
    (setq exit-alist (cl-union (emud-room-exits room1) (emud-room-exits room2)))
    (while  exit-alist
...
(defun emud-map-fix-entrances2 (map new-room old-room)
  (let ((new-number (emud-room-number new-room))
	(old-number (emud-room-number old-room))
	direction
	exit-cons
	exit-room
	entrances)

    (dolist (exit-cons (cl-union (emud-room-exits new-room)
				   (emud-room-exits old-room)))
      (setq direction (car exit-cons)) 
      (setq dest-room (emud-map-get-room map (cdr exit-cons)))
      (when (and dest-room 
	     (setq entrances (assq direction (emud-room-entrances dest-room)))
	     (member old-number entrances))
	(delete old-entrance entrances)
	(unless (member new-number entrances)
	  (nconc entrances (list new-number))
	   (when (cddr entrances); (> 2 (length entrances))
	     (emud-warn "Suspicious entrance list")))))))
...
(defun emud-map-merge-rooms  (map
  ...
  (let ( main-exits
    ...
    (setq merge-list
	  (cl-union (emud-map-merge-exits main-room target-room)
...
(defun emud-map-merge-by-direction (map main-room)
  ...
  (let ((main-entrances (emud-room-entrances main-room))
	...
	merge-number
					; number for room to be merged with main
	main-number
	...
	(when main-entrance
	  ...
	  (setq merge-candidates
		(cl-intersection (emud-room-siblings dest-room) main-entrance))
	  ...
	  (when (and merge-candidates (not (eq dest-flag 'maze)))
	    ...
	    (setq merge-candidates (cl-union (list (cdr main-exit))
					  merge-candidates))
	    ...
	    ;; merge-candidate
	    (dolist (merge-number merge-candidates)
	      (setq merge-list (cons (cons main-number merge-number)
				     merge-list))))))
...
(defun emud-map-merge-exits (main-room target-room)
  (let ( main-exits
    ...
    (setf (emud-room-exits main-room) 
	  (cl-union main-exits target-exits :test 'equal))
	...
	(cond 
	 ...
	 ((> (cdr target-exit) (cdr main-exit))
	  ...
	  (setq merge-list
		(cons  (cons (cdr main-exit) (cdr target-exit)) merge-list)))
	...
	;; in accending order
	 (t
	  ...
	  (setq merge-list
		(cons (cons (cdr target-exit) (cdr main-exit)) merge-list))))))
    merge-list))
...
(defun emud-map-merge-entrances (map main-room target-room)
  (let (main-entrances
    ...
    (dolist (main-entrance main-entrances)
	...
	(setq source-list (cl-union (cdr main-entrance)
				 (cdr target-entrance)))
	  ...
	  (cond
	   ...
	   (source-room
	    ...
	    (setq source-siblings
		  (cl-intersection (emud-room-siblings source-room)
				source-list))
	...
	(setq merged-entrances (cons  (cons (car main-entrance)
					       main-list)
				      merged-entrances))) 
	...
	(setq merged-entrances (cons main-entrance  merged-entrances )))))
				       

    
    ;; Add any remaiing target entrances to the merged entrance list
    (setq merged-entrances (nconc merged-entrances
				  target-entrances))
...
check it's sibling list for room with appropriate coordinates"
  (let ((siblings (emud-map-get-siblings map short))
    ...
    (when (and
      ...
      (setq siblings (cl-remove-if-not check-function (cdr siblings)))
      (unless (cdr siblings)
	(car siblings)))))
  ...
  (let ((path (emud-map-walk2 emud-curr-map (emud-room-number emud-map-curr-room) to))
    ...
    (when (and (not path)
     ...
     (while path
	   ...
	   (setq rev-path (cons step rev-path))
	 (setq path nil
	       rev-path nil)))
     (setq path rev-path))
...
(defun emud-map-delete-room (map room)
  (let ((number (emud-room-number room))
    ...
    (setf (emud-map-hole map) (cons number hole ))
...
(defun emud-map-update-room ()
  (let ((buffer (get-buffer-create "*Emud-room*")))
    (emud-map-room-summary emud-curr-map emud-map-curr-room buffer)))
...
(defun emud-map-room-summary (map room buffer)
  (with-current-buffer buffer
	...
	(insert (format "(%3d) <%s> %s %s\n"
			...
			(emud-room-obv-exits room)))
	(insert "Exits:\n")
	(dolist (room-exit (emud-room-exits room))
	  (when (integerp (cdr room-exit))
...
(defun emud-map-calc-coord (map room)
  (let ((origin (assq 'origin (emud-map-alias-list map)))
	(coord [0 0 0])
	direction)
    (dolist (direction (emud-map-walk2 map origin room))
      (setq coord (or (emud-map-walk-coord direction coord) coord)))
    coord))

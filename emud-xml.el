...
(defun emud-find-xml-region (buffer start)
  (let (region-start
	region-end 
	node-name)
    (save-excursion
      ...
      (cond
       ((= (point) (point-max))
	nil)
       ((eq (char-after) ?\n) ; make newlines an xml line-break
 	(setq region-start (point))
 	(insert "<BR/>")
 	(list region-start (1+ (point))))
       ((looking-at "[ \n\t\r]")
	(setq region-start (point))
	(insert "<SPACE space=\"")
	(skip-chars-forward " \n\t\r")
	...
	(cond 
	 ...
	 ((re-search-forward (concat "</" node-name "\\s-*>") (point-max) t)
	  (setq region-end (point))
	  (while (looking-at (concat "[^<]*</" node-name "\\s-*>"))
	    (setq region-end (match-end 0))
	    (search-backward "</" region-start)
	    (setq region-end (emud-unxml-tag region-end))
	    (goto-char region-end))
	  (goto-char region-start)
	  (while (search-forward ">" (1- region-end) t)
	    (when (looking-at "\\s-")
	      (insert "<SPACE space=\"")
	      (skip-chars-forward " \t\n\r")
	      (insert "\"/>")
	      (setq region-end (+ region-end (length "<SPACE space=12/>")))))
	  (goto-char region-end)

;;xml.el is buggy and thinks any whitspace trailing an end tag
;;is PART of the end-tag!!!
;;so replace ret with <BR/>
	   (cond
	    ((looking-at "[\t ]")
	     (insert "<SPACE space=\"")
	     (skip-chars-forward " \t\n")
	  ...
	  (;(and (not (string= node-name "prompt"))
	   (or (search-forward "<prompt>" (point-max) t)
	   ...
	   (goto-char region-start)
	   (emud-unxml-tag (point-max))
	   (emud-find-xml-region buffer start))
	  (t ;; can't find an end tag :(
	   nil)))
       ((eq (char-after) ?<)           ;spurious < ?
	(emud-unxml-tag (point-max))   ;fix it
	(emud-find-xml-region buffer start)) ; and try again.

       ((looking-at emud-prompt-pattern) ;are we looking at a prompt?
	(setq region-start (point))
	(insert "<PROMPT>")
	(goto-char (+ (match-end 0) (length "<prompt>")))
	(insert "</PROMPT>")
	(list region-start (point)))
       ;(looking-at "^.*died.")
       (t
...
(defun emud-buffer-point-max (buffer)
  (save-excursion
    (set-buffer buffer)
    (point-max)))

(defun emud-parse-xml-buffer (proc xml-curr-char xml-buffer) 
  (let (region xml-list xml region-start region-stop)
    
    (setq region 
	  (emud-find-xml-region xml-buffer xml-curr-char))
    (while region
      (setq region-start (car region))
      (setq region-stop (car (cdr region)))
      (setq xml-list (emud-xml-parse-region 
		      region-start region-stop xml-buffer))
	...
	(run-hooks 'emud-new-command-hook))
      (setq region (emud-find-xml-region xml-buffer xml-curr-char)))
    xml-curr-char))

(defun emud-xml-parse-region (region-start region-stop xml-buffer)
  (let (err err-msg node)
    (condition-case err
	(list 
	 (xml-parse-region 
	  region-start region-stop xml-buffer) region-start region-stop)
      (error 
       (setq err-msg (error-message-string err))
       ;(debug)
       (point)
       (message "%d [%d %d] %s\n" (point) region-start region-stop err-msg)
       (cond 
	((or (string= err-msg "XML: Invalid attribute list")
	     (string= err-msg "XML: Invalid character")
	     (string-match "^XML: (Well-Formed) Couldn.t parse tag:" err-msg))
	 ;; probably means someone used a < 
	 (save-excursion
	   (set-buffer xml-buffer)
	   (if (or (eq (char-after) ?<) 
		   (re-search-backward "<[^<>]+<" region-start t)
		   (search-backward "<" region-start))
	       (progn 
		 (setq region-stop (emud-unxml-tag region-stop))
		 (emud-xml-parse-region 
		  region-start region-stop xml-buffer))
	     (error "%s at pos %d" err-msg (point)))))
	((or 
	 (string-match "^XML: (Not Well-Formed) End of document while reading element .\\([a-zA-Z]+\\)" err-msg)
	 (string-match 
	  (concat "^XML:.*nvalid end tag (expecting \\([^)]+\\))" 
		  " at pos \\([0-9]+\\)" ) err-msg))
	 (setq node (match-string-no-properties 1 err-msg))
	 (message (concat "Caught unended tag " node "."))
	 (save-excursion
	   (set-buffer xml-buffer)
	   (search-backward (concat "<" node) (1+ region-start) t)
	   (setq region-stop (emud-unxml-tag region-stop)))
	 (emud-xml-parse-region 
	  region-start region-stop xml-buffer))
	(t (error "xml-parse-region: %s" err-msg)))))))
  ...
  (let (region-start
	...
	(cond 
	 ...
	 ((re-search-forward (concat "</" node-name "\\s-*>") (point-max) t)
	  ...
	  (search-backward "</" region-start)
	  ;;(forward-char -1)
	  (setq arg-string (buffer-substring-no-properties arg-start (point)))
	  (setq xml (append xml (list arg-string)))
	  (list xml region-start region-end))
	 (;(and (not (string= node-name "prompt"))
	  (or (re-search-forward "<prompt>" (point-max) t)
	...
	(goto-char (match-end 0))
	(setq arg-string (buffer-substring-no-properties region-start (point)))
	(setq xml (list 'PROMPT nil arg-string))
	;;(insert "</PROMPT>")
	(list xml region-start (point)))
       ;(looking-at "^.*died.")
       (t
